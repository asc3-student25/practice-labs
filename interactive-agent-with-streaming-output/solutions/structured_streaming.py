...
async def stream_structured_character(description: str) -> CharacterProfile:
    ...
    Demonstrates:
    - Streaming partial parses via response.stream_output(...)
    - Extracting the final validated model via await response.get_output()
    - Type validation with Pydantic

    Note: response.stream_text() raises UserError on a structured agent —
    text-only streaming is for agents WITHOUT output_type=. For typed
    output, use stream_output() (yields partial model instances) and
    get_output() (returns the final validated instance).
    """
    print(f"Creating character: {description}\n")
    print("Generating: ", end="", flush=True)

    async with character_agent.run_stream(description) as response:
        # Surface partial parses as fields arrive
        async for _partial in response.stream_output(debounce_by=0.1):
            print(".", end="", flush=True)

        print("\n\n--- Structured Output ---")
        # Extract the validated structured data inside the async-with
        # block — once it exits, response.get_output() is unreachable.
...
async def stream_structured_plot(premise: str) -> PlotOutline:
    ...
    async with plot_agent.run_stream(premise) as response:
        ...
        print("\n\n--- Structured Output ---")
        plot = await response.get_output()
...
async def stream_structured_setting(location: str) -> SceneSetting:
    ...
    async with setting_agent.run_stream(location) as response:
        ...
        print("\n\n--- Structured Output ---")
        setting = await response.get_output()
