...
@mcp.tool()
def rotate_log_file(filename: str) -> str:

    """This tool archives the specified log file by appending a UTC timestamp to its name and WRITES a new empty file in its place.

    Do NOT use this tool for reading log files, searching, counting, or listing files.

    Use this tool when the user wants to archive an existing log file and start fresh, such as "rotate the application.log file to archive old logs and create a new empty application.log".


    Args:
        filename: The name of the log file to rotate (relative to the log directory)."""
