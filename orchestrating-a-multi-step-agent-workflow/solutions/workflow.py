...
class WorkflowOrchestrator:
    ...
    async def _execute_step(self, step: WorkflowStep) -> str:
        ...
        # YOUR CODE HERE
        # [SOLUTION]
        # Format prompt with context data. Catch KeyError so the message
        # names the step and missing key rather than just the key.
        try:
            prompt = step.prompt_template.format(**self.context.data)
        except KeyError as missing:
            raise KeyError(
                f"Step '{step.name}' prompt_template references "
                f"{missing!s} but no such key exists in WorkflowContext.data. "
                f"Add {missing!s} to step.required_inputs (or ensure a "
                f"prior step writes step_<name>_result with that key)."
            ) from missing
