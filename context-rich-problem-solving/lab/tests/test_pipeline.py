...
def test_ingest_event_builds_event_from_raw():
    raw = {"customer_id": "u42", "type": "CLICK", "ts": "2026-01-01T00:00:00Z", "value": "1.5"}
    event = ingest_event(raw)
    assert event.customer_id == "u42"
...
def test_normalize_lowercases_event_type():
    event = ingest_event(
        {"customer_id": "u1", "type": " PAGE_VIEW ", "ts": "2026-01-01T00:00:00Z"}
    )
...
def test_to_jsonl_serializes_customer_id_and_schema():
    event = normalize(
        ingest_event({"customer_id": "u7", "type": "click", "ts": "2026-01-01T00:00:00Z"})
    )
    output = to_jsonl([event])
    record = json.loads(output)
    assert record["customer_id"] == "u7"
