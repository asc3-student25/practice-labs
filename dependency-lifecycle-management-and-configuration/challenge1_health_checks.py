...
class HealthChecker:
    ...
    async def check_database(self) -> HealthCheckResult:
        ...
        try:
            ...
            elif hasattr(db, "fetchval"):
                await db.fetchval("SELECT 1")
            elif hasattr(db, "fetch_one"):
                await db.fetch_one("SELECT 1")
            else:
                raise RuntimeError("Database client does not support a health-check query method")
    ...
    async def check_cache(self) -> HealthCheckResult:
        ...
        try:
            ...
            key = "healthcheck:sentinel"
            value = f"ok:{int(start * 1000)}"
            ttl_seconds = 5
            ...
            if cached_value != value:
                raise ValueError("Cache round-trip value mismatch")
    ...
    async def check_all(self) -> Dict[str, HealthCheckResult]:
        ...
        """
        database_result = await self.check_database()
        cache_result = await self.check_cache()
        http_result = await self.check_http_client()
