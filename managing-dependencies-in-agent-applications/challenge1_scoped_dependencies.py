...
class ScopedDependencyContainer:
    ...
    async def dispose(self):
        ...
        """
        # Dispose instantiated singleton dependencies
        for name, registration in self._registrations.items():
            if (
                registration.scope == DependencyScope.SINGLETON
                and registration.singleton_instance is not None
            ):
                await self._dispose_instance(registration.singleton_instance, name)

        # Dispose cached request-scoped dependencies
        for name, instance in self._request_instances.items():
            await self._dispose_instance(instance, name)

        # Clear all cached/registered dependencies
