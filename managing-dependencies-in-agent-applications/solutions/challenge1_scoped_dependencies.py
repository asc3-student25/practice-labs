    ...
    async def resolve(self, name: str) -> Any:
        ...
        if registration.scope == DependencyScope.SINGLETON:
            if registration.singleton_instance is None:
                logger.debug(f"Creating singleton instance of '{name}'")
                registration.singleton_instance = await self._create_instance(
                    registration.factory
                )
            else:
                logger.debug(f"Reusing singleton instance of '{name}'")
        ...
        # Handle request scope
            ...
            if name not in self._request_instances:
                logger.debug(f"Creating request-scoped instance of '{name}'")
                self._request_instances[name] = await self._create_instance(
                    registration.factory
                )
            else:
                logger.debug(f"Reusing request-scoped instance of '{name}'")
            return self._request_instances[name]

        # Handle transient scope
        else:
            logger.debug(f"Creating transient instance of '{name}'")
