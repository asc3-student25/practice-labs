...
async def get_order(ctx: RunContext[AgentDependencies], order_id: str) -> OrderInfo:
    ...
    Returns:
        Order information
    """
    logger.info(f"Searching order by ID: {order_id}")

    cache_key = f"order:{order_id}"
    cached_order = await ctx.deps.cache.get(cache_key)
    if cached_order is not None:
        return OrderInfo.model_validate(cached_order)
...
async def main():
    ...
    try:
        ...
        queries = [
            ...
            "Find all orders for customer alice@example.com",
        ]
